/*
program: rxMulch.cmd
type:    REXXSAA-OS/2, version 2.x, or REXXSAA 6.0 (Object REXX)
...
changed: 1995-04-05, rgf, changed a bug related to renaming an already
                          deleted tmp-file (because no changes took place)
         1995-09-22, rgf, adjusted for checking if running under Object REXX; if so
                          using COUNTSTR() and CHANGESTR() instead, which speeds up
...
PARSE VERSION . global.eVersion .       /* get REXX-version */
...
DO
   ...
   IF global.eFileout = "" & writeFile THEN
   DO
      global.eTmp = SysTempFileName("tmp???.rxm")
...
DO
   IF global.eTotalOfChanges > 0 THEN           /* changes took place */
   DO
...
WORK_ON_TMPVAR1: PROCEDURE EXPOSE global. tmpVar1 tmpVar2 bDirty
    ...
    IF global.eVersion >= 6 THEN        /* running under Object REXX */
    DO
       tmpCount = COUNTSTR(from_needle, tmpVar1)
       bDirty = tmpCount > 0
       IF bDirty THEN
       DO
          global.eTotalOfChanges = global.eTotalOfChanges + tmpCount
          tmpVar2 = CHANGESTR(from_needle, tmpVar1, to_needle)
       END
    END
    ELSE                                /* running under traditional REXX */
    DO
...
WORK_ON_TMPVAR2: PROCEDURE EXPOSE global. tmpVar2 tmpVar1 bDirty
    ...
    IF global.eVersion >= 6 THEN        /* running under Object REXX */
    DO
       tmpCount = COUNTSTR(from_needle, tmpVar2)
       bDirty = tmpCount > 0
       IF bDirty THEN
       DO
          global.eTotalOfChanges = global.eTotalOfChanges + tmpCount
          tmpVar1 = CHANGESTR(from_needle, tmpVar2, to_needle)
       END
    END
    ELSE                                /* running under traditional REXX */
    DO

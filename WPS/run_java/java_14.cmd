...
/*------------------------------------------------------------------*/
versionstring = "Carys java_14.cmd ver 20030223"
crlf = '0D0A'X
jar_file=''
java_arg=''
...
/* Look for the location of Java141 in the os2.ini file                    */
/* Since most OS/2 users don't have java14x their default java, a          */
...
if jpth = "ERROR:" then do                    /* Java14 runtime not found     */
      ...
      if jpth = "ERROR:" then do              /* Java14 SDK not found         */
         say "Java14 not found -- Use Java 13 script";
         exit 1;
      end; /* then-do */
      else
         do
         jbase =Strip(jpth,,D2C(00));
...
/* launch the app                                                          */
jbase ||'\bin\java.exe -cp '|| jar_path ||' '||java_arg
...
say "Usage: java_14 [java options] [-jar] jarpath\jarfile.jar [jar options]"crlf
say "java_14 is used to run java14x based programs"crlf

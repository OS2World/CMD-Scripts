...
parse arg server user password newmailfilemask

if (server = "") | (user='') | (password='') | (newmailfilemask='') then
   do
         ...
         then say ' There is no message waiting for you.'
           ...
           do 
             ...
             do i = 1 to messages
               ...
               trc = GetResponse(sock)
               parse var line.1 status rest
               newMailFile = SysTempFileName(newmailfilemask)
               if newmailfile = '' then newmailfile = newmailfilemask
               oneline = GetResponseLine(sock)
               do while oneline <> '.'
                 call lineout newMailFile, oneline
                 oneline = GetResponseLine(sock)
               end
